...
#include "Pin_CLK.h"
    ...
    #define Pin_CLK_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin_CLK_PC =   (Pin_CLK_PC & \
                                (uint32)(~(uint32)(Pin_CLK_DRIVE_MODE_IND_MASK << \
                                (Pin_CLK_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin_CLK_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin_CLK_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin_CLK_USBIO_CTRL_REG = (Pin_CLK_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin_CLK_DRIVE_MODE_IND_MASK << \
                                    (Pin_CLK_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin_CLK_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin_CLK_SetP4PinDriveMode(Pin_CLK__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Pin_CLK_DR & (uint8)(~Pin_CLK_MASK));
    drVal = (drVal | ((uint8)(value << Pin_CLK_SHIFT) & Pin_CLK_MASK));
    Pin_CLK_DR = (uint32)drVal;
}
...
{
    return (uint8)((Pin_CLK_PS & Pin_CLK_MASK) >> Pin_CLK_SHIFT);
}
...
{
    return (uint8)((Pin_CLK_DR & Pin_CLK_MASK) >> Pin_CLK_SHIFT);
}
...
void Pin_CLK_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin_CLK_INTCFG & (uint32)(~(uint32)position);
    Pin_CLK_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Pin_CLK_INTSTAT & Pin_CLK_MASK);
	Pin_CLK_INTSTAT = maskedStatus;
    return maskedStatus >> Pin_CLK_SHIFT;
}
