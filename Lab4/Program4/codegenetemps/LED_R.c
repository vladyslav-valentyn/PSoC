...
#include "LED_R.h"
    ...
    #define LED_R_SetP4PinDriveMode(shift, mode)  \
    do { \
        LED_R_PC =   (LED_R_PC & \
                                (uint32)(~(uint32)(LED_R_DRIVE_MODE_IND_MASK << \
                                (LED_R_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (LED_R_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define LED_R_SetP4PinDriveMode(shift, mode)  \
        do { \
            LED_R_USBIO_CTRL_REG = (LED_R_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(LED_R_DRIVE_MODE_IND_MASK << \
                                    (LED_R_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (LED_R_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		LED_R_SetP4PinDriveMode(LED_R__0__SHIFT, mode);
    }
#endif
...
void LED_R_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  LED_R_INTCFG & (uint32)(~(uint32)position);
    LED_R_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
